...
def pe_based_sip_model (company, installment, start_year) :
    ...
    while row_num < len(stock):
        ...
        # PRECAUTIOUS SELL : Sell 30% of Portfolio on R2 crossover
        
        elif ((nifty500['P/E'].iloc[row_num] > confidence_intervals[4]) and (number_of_successive_sip >= 12)) :
    ...
    writer = pd.ExcelWriter(company + ' (PE Based S-1).xlsx', engine = 'xlsxwriter') # Create a Pandas Excel Writer using XLSXWriter as the engine.
